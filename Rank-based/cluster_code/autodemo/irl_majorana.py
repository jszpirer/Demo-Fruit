...
def compute_phi(fsm, argos, seed):
...
def run_Argos(fsm, argos, seed):
    # run argos with passed fsm to write hitory file
    with open("./argos.sh",'w+') as f:
        f.write("#!/usr/bin/env python\n")
        f.write(f"/home/jszpirer/Mandarina/AutoMoDe-tuttifrutti/bin/automode_main -n -c {argos} --seed {seed} --fsm-config {fsm}\n")
...
def computeScore(fsm, w, argos, seed):
    print("On compute le score")
    phi = compute_phi(fsm, argos, seed)
    print(phi)
    print(w)
    score_blue = np.dot(np.array(w[0]).T, phi[0])
    score_red = np.dot(np.array(w[1]).T, phi[1])
...
if __name__ == '__main__':
    ...
    fsm = sys.argv[1]
    seed = sys.argv[2]
    score_blue, score_red = computeScore(fsm, w, argos, seed)

    print(f"BlueObjective {score_blue}")
    print(f"RedObjective {score_red}")
