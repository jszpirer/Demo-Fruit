...
if __name__ == "__main__":
    ...
    for file in os.listdir(f"../JSON/{mission}_{date}/distance"):
        if file.split(".")[-1] == "json":
            ...
            for i in range(1, len(info)):
                ...
                if i == len(info)-1:
                    info_final_plot.append({"exp":exp,"distance part1":distance_part1_mean, "distance part2":distance_part2_mean,"mean distance":best_mean_distance, "fsm":fsm})
                    exp+=1
    ...
    for j in range(nb_designs):
        ...
        ax1 = plt.subplot(1, 2, 1)
        a = np.array(w[0])
        h2 = sns.heatmap(a.reshape(len(w[0]), 1), center=0, ax=ax1, cmap=colormap, yticklabels=5, xticklabels=False)
        cbar2 = h2.collections[0].colorbar
        cbar2.ax.set_yticklabels(cbar2.ax.get_yticklabels(), rotation=90, va='center')
        ax1.invert_yaxis()
        ax1.set_ylabel("Circle (0-19)       Black right (20-39)       Black left (40-59)     Neighbor(60-79)",  fontsize=8) #aac
        ...
        ax2 = plt.subplot(1, 2, 2)
        a = np.array(w[1])
        h2 = sns.heatmap(a.reshape(len(w[0]), 1), center=0, ax=ax2, cmap=colormap, yticklabels=5, xticklabels=False)
        cbar2 = h2.collections[0].colorbar
        cbar2.ax.set_yticklabels(cbar2.ax.get_yticklabels(), rotation=90, va='center')
        ax2.invert_yaxis()
        ax2.set_ylabel("Circle (0-19)       Black right (20-39)       Black left (40-59)     Neighbor(60-79)",  fontsize=8) #aac
        ...
        plt.savefig(f"../JSON/{mission}_{date}/heatmap_"+mission+""+str(j)+".png")
